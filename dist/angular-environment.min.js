...
return this.data.vars[this.get()][variable];};this.is=function(environment){return(environment===this.environment);};this.check=function(){var	self=this,location=local.host?local.host:window.location.host,matches=[],keepGoing=true;angular.forEach(this.data.domains,function(v,k){angular.forEach(v,function(v){if(location.match(local.stringToRegex(v))){matches.push({environment:k,domain:v});}});});angular.forEach(matches,function(v,k){if(keepGoing){if(location===v.domain){keepGoing=false;}
void 0;self.environment=v.environment;}});local.hasChecked=true;};this.$get=function($location){if(!local.hasChecked){local.host=$location.host();this.check();}
return this;};this.$get.$inject=['$location'];});